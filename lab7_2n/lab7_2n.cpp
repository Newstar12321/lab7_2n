...
int main()
{
    ...
    case 1: cout << array[0];
        break;
    case 2: if (array[0] < array[1]) array[0] *= array[0];
          else array[1] *= array[1];
        break;
    default: ar_sqrt(n, array);
        break;
    }
...
void ar_new(size_t n, int* array, int* new_array)
{
    for (size_t i = 0; i < n; i++)
    {
        new_array[i] = array[i];
